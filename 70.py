...
def play(conn, gameData):
    my_x = gameData.balls[0][0]
    my_y = gameData.balls[0][1]
    for i in range(1, NUMBER_OF_BALLS):
        other_x = gameData.balls[i][0]
        other_y = gameData.balls[i][1]
        if other_x == 0 and other_y == 0:
            continue
        diff_x = other_x - my_x
        diff_y = other_y - my_y
        distance = math.sqrt(diff_x ** 2 + diff_y ** 2)
        temp = math.degrees(math.atan2(diff_y, diff_x))
        angle = 90 - temp
        power = 100 / (81) * distance
        if power < 50:
            power += 50
